...
void main() {
    ...
    vec2 texel = vec2(1.0) / vec2(texSize);
    vec2 blurRadius = texel * 7.0;
    vec2 cellInset = texel * 9.0;
    vec2 cellMin = cellOffset + cellInset;
    vec2 cellMax = cellOffset + cellSize - cellInset;

    vec4 blurred = texture(uTex, clamp(st, cellMin, cellMax)) * 0.20;
    blurred += texture(uTex, clamp(st + vec2( blurRadius.x, 0.0), cellMin, cellMax)) * 0.12;
    blurred += texture(uTex, clamp(st + vec2(-blurRadius.x, 0.0), cellMin, cellMax)) * 0.12;
    blurred += texture(uTex, clamp(st + vec2(0.0,  blurRadius.y), cellMin, cellMax)) * 0.12;
    blurred += texture(uTex, clamp(st + vec2(0.0, -blurRadius.y), cellMin, cellMax)) * 0.12;
    blurred += texture(uTex, clamp(st + vec2( blurRadius.x,  blurRadius.y), cellMin, cellMax)) * 0.08;
    blurred += texture(uTex, clamp(st + vec2(-blurRadius.x,  blurRadius.y), cellMin, cellMax)) * 0.08;
    blurred += texture(uTex, clamp(st + vec2( blurRadius.x, -blurRadius.y), cellMin, cellMax)) * 0.08;
    blurred += texture(uTex, clamp(st + vec2(-blurRadius.x, -blurRadius.y), cellMin, cellMax)) * 0.08;

    float monochrome = dot(blurred.rgb, vec3(0.299, 0.587, 0.114));
    monochrome = mix(monochrome, 1.0, 0.06);
    outColor = vec4(vec3(monochrome), blurred.a * vAlpha);
}
`;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  const [activeItem, setActiveItem] = useState(
    () => (items.length ? items : defaultItems)[0]
  );
  ...
  const handleButtonClick = () => {
    ...
    const destination = new URL(activeItem.link, window.location.href);
    window.open(destination.href, '_blank', 'noopener,noreferrer');
  };

  const isInternshipTitle = activeItem?.title === 'Internship Experience';
  ...
  return (
    <div className="infinite-menu" style={{ position: 'relative', width: '100%', height: '100%' }}>
      <canvas
        ...
        ref={canvasRef}
        aria-label="Drag to explore selected work"
      />

      {activeItem && (
        <>
          <h2
            className={`face-title${isInternshipTitle ? ' multiline' : ''} ${
